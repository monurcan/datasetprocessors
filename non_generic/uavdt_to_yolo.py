...
def convert_bbox(bbox_infos):
    ...
    x1, y1, w, h = int(bbox_infos[0]), int(
        bbox_infos[1]), int(bbox_infos[2]), int(bbox_infos[3])
    x_center = x1+w/2
    y_center = y1+h/2
    x_center /= 1024
    y_center /= 540
...
def get_info_from_line(infos):
    ...
    x_center, y_center, w, h = convert_bbox(
        [infos["<bbox_left>"], infos["<bbox_top>"], infos["<bbox_width>"], infos["<bbox_height>"]])
    # 0- car,  1 - truck, 2- bus
...
def get_annot_data(txt_file):
    ...
    annot_data = pd.read_csv(txt_file, delimiter=",", names=["<frame_index>", "<target_id>", "<bbox_left>",
                             "<bbox_top>", "<bbox_width>", "<bbox_height>", "<out-of-view>", "<occlusion>", "<object_category>"])
    return annot_data


def get_infos(frame_id):
    """Extract sub-dataframe w.r.t frame_id - get only box coordiantes and object caterogy"""
    infos = annot_data[annot_data["<frame_index>"] == frame_id].loc[:, [
        "<bbox_left>", "<bbox_top>", "<bbox_width>", "<bbox_height>", "<object_category>", "<occlusion>", "<out-of-view>"]]
    return infos
...
def create_annot(img_path, infos):
    ...
    with open(annot_path, "w") as f:
        for row in infos.iterrows():  # Fix
            ...
            if(row[1]['<occlusion>'] != 2):  # large occlusion
...
if __name__ == "__main__":
    ...
    root_dir = "/mnt/RawInternalDatasets/KaraGAG/Fujinon/FromWeb/UAVDT/UAV-benchmark-M"
