...
class YOLOV5CFG2COCO:
    ...
    def generate(self):
        self.train_files = self.get_files(self.train_path, self.dst / self.coco_annotation / f"train.npy")
        self.valid_files = self.get_files(self.val_path, self.dst / self.coco_annotation / f"val.npy")
    ...
    def get_files(self, path, npy_save_path):
        ...
        for p in path:
            ...
            dataset_files = natsorted(
                x.replace("/", os.sep) for x in files if x.split(".")[-1].lower() in IMG_FORMATS
            )
            
            im_files += dataset_files
            npy_im_files.append(dataset_files)
            
        npy_im_files = np.array(npy_im_files, dtype=object)
        np.save(npy_save_path, npy_im_files)
